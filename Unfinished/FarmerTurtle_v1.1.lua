-- Farmer Turtle settings and work v1.1
-- Code by SibFox
...
local tPlantData = {
    ...
    ["Onion"] = { Name = "farmersdelight:onion", Block = "farmersdelight:onions", Seeds = "farmersdelight:onion", MaxAge = 7 },
...
local tBlockData = {
    ...
    ["farmersdelight:cabbages"] = "Cabbage",
    ["farmersdelight:onions"] = "Onion",
    ["farmersdelight:tomatoes"] = "Tomato"
}
...
while true do
    print("> Welcome to FarmerTurtle v1.1 settings!\n")
    ...
    if type(iFarmLength) ~= "number" or type(iFarmWidth) ~= "number" then
        ...
        elseif iFarmLength * iFarmWidth > 100 then
            clearTerm(1,1)
            print("err> Maximum planation size is 100")
...
local function getPlantData(tData)
    return tPlantData[tBlockData[tData.name]]
end
...
while true do
    ...
    for w = 0, iFarmWidth do
        for l = 0, iFarmLength do
            local bSuccess, tData = turtle.inspectDown()
            if bSuccess then
                if tData.state.age == getPlantData(tData).MaxAge then
                    turtle.digDown()
                    turtle.suckDown()
                else
                    print("inf> Not a grown plant. Age "..tostring(tData.state.age).. " of max "..tostring(getPlantData(tData).MaxAge))
                end
                if turtle.getItemDetail().name == getPlantData(tData).Seeds then
